...
void AS7331::powerDown() {
	...
	_i2c_bus->writeByte(AS7331_ADDRESS, AS7331_OSR, temp & (uint8_t)~(0x40) ); // clear bit 6
}

void AS7331::powerUp() {
	uint8_t temp = _i2c_bus->readByte(AS7331_ADDRESS, AS7331_OSR);  // Read the raw data register
	Serial.print((uint8_t*)"Power up routine: ");
	Serial.println(temp);
...
//   1 means open drain
	_i2c_bus->writeByte(AS7331_ADDRESS, AS7331_CREG1, (uint8_t)(gain << 4) |  time ); //
	_i2c_bus->writeByte(AS7331_ADDRESS, AS7331_CREG3, (uint8_t)(mmode << 6) | (uint8_t)(sb << 4) | cclk ); //
